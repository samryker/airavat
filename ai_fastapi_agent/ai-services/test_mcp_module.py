"""
Comprehensive Tests for MCP (Model Context Protocol) Medical Agent
Tests every function and user contextual data storage functionality
"""
...
class TestRealGeminiLLM:
    ...
    async def test_real_gemini_llm_ainvoke_string(self, mock_genai):
        ...
        assert result.content == "I recommend rest and hydration"
...
class TestMCPMedicalAgentInitialization:
    ...
    @patch('main_agent.mcp_medical_agent.LANGGRAPH_AVAILABLE', True)
    @patch('main_agent.mcp_medical_agent.MemorySaver')
    @patch('main_agent.mcp_medical_agent.StateGraph')
    def test_mcp_agent_initialization_with_langgraph(self, mock_state_graph, mock_memory_saver):
        """Test MCPMedicalAgent initialization with LangGraph available"""
        mock_db_engine = Mock()
        mock_firestore = Mock()
        ...
        assert agent.agent_graph is not None
...
class TestMCPAgentGraph:
    """Test MCP agent graph creation and workflow"""
    
    @patch('main_agent.mcp_medical_agent.LANGGRAPH_AVAILABLE', True)
    @patch('main_agent.mcp_medical_agent.MemorySaver')
    @patch('main_agent.mcp_medical_agent.StateGraph')
    def test_create_agent_graph(self, mock_state_graph, mock_memory_saver):
    ...
    @patch('main_agent.mcp_medical_agent.LANGGRAPH_AVAILABLE', False)
    def test_create_agent_graph_fallback(self):
        """Test agent graph creation when LangGraph is not available"""
        agent = MCPMedicalAgent()
        
        # Should return None when LangGraph is not available
        assert agent.agent_graph is None
...
class TestMCPContextLoading:
    ...
    async def test_load_patient_context_with_firestore(self):
        ...
        mock_firestore = Mock()
        mock_firestore.get_complete_patient_context.return_value = {
...
class TestMCPContextSummary:
    ...
    def test_create_context_summary_empty(self):
        ...
        assert "No profile data available" in summary
        assert "No treatment history available" in summary
        assert "No biomarker data available" in summary
...
class TestMCPQueryAnalysis:
    ...
    async def test_analyze_query(self):
        ...
        updated_state = await agent._analyze_query(state)
        
        assert "query_analysis" in updated_state
        assert "urgency_level" in updated_state["query_analysis"]
        assert "intent" in updated_state["query_analysis"]
        assert "key_symptoms" in updated_state["query_analysis"]
...
class TestMCPResponseGeneration:
    ...
    async def test_generate_response_with_gemini(self, mock_gemini):
        ...
        updated_state = await agent._generate_response(state)
        
        assert "ai_response" in updated_state
        assert "immediate medical attention" in updated_state["ai_response"]
    ...
    async def test_generate_response_without_gemini(self):
        ...
        assert "ai_response" in updated_state
        assert len(updated_state["ai_response"]) > 0
...
class TestMCPMemoryManagement:
    ...
    async def test_update_memory(self):
        ...
        updated_state = await agent._update_memory(state)
        
        assert "memory_updated" in updated_state
        assert updated_state["memory_updated"] is True
    ...
    async def test_save_context(self):
        ...
        updated_state = await agent._save_context(state)
        
        assert "context_saved" in updated_state
        assert updated_state["context_saved"] is True
...
class TestMCPDataExtraction:
    ...
    def test_extract_treatment_preferences(self):
        ...
        assert "natural" in str(preferences).lower()
        assert "medication" in str(preferences).lower()
    ...
    def test_extract_health_goals(self):
        ...
        assert "cardiovascular" in goals
        assert "health" in goals
...
class TestMCPQueryProcessing:
    ...
    async def test_process_query_with_agent_graph(self, mock_gemini):
        ...
        agent = MCPMedicalAgent()
        agent.agent_graph = mock_agent_graph
        ...
        assert "rest and hydration" in response.response_text
    ...
    async def test_process_query_fallback(self, mock_gemini):
        ...
        agent = MCPMedicalAgent()
        agent.agent_graph = None  # Force fallback
...
class TestMCPDynamicSuggestions:
    ...
    async def test_generate_dynamic_suggestions_with_gemini(self, mock_gemini):
        ...
        assert suggestions[0].suggestion_text == "Take rest and stay hydrated"
        assert suggestions[0].confidence_score == 0.8
        assert suggestions[1].suggestion_text == "Monitor your symptoms"
        assert suggestions[1].confidence_score == 0.7
    ...
    async def test_generate_dynamic_suggestions_fallback(self):
        ...
        assert any("consultation" in s.suggestion_text.lower() for s in suggestions)
        assert any("monitor" in s.suggestion_text.lower() for s in suggestions)
        assert any("follow up" in s.suggestion_text.lower() for s in suggestions)
...
class TestMCPIntegration:
    ...
    async def test_full_conversation_flow(self, mock_gemini):
        ...
        agent = MCPMedicalAgent(firestore_service=mock_firestore)
        agent.agent_graph = None  # Use fallback implementation
        ...
        assert "headache" in response1.response_text.lower()
        ...
        assert "rest and hydration" in response2.response_text.lower()
    ...
    async def test_emergency_scenario_handling(self):
        ...
        assert "immediate" in response.response_text.lower() or "emergency" in response.response_text.lower()
        assert len(response.suggestions) > 0
        
        # Check that suggestions include emergency actions
        suggestion_texts = [s.suggestion_text.lower() for s in response.suggestions]
        assert any("emergency" in text or "immediate" in text or "doctor" in text for text in suggestion_texts)
