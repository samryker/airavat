...
"""
        ...
        # 5) Persist a physician-friendly record into treatment_plans for longitudinal review
        if FIREBASE_AVAILABLE and db is not None:
            try:
                plan_doc = {
                    "request_id": f"{int(datetime.utcnow().timestamp()*1000)}",
                    ...
                    "patient_id": patient_id,
                    "treatment_text": gemini_resp.text,
                    "file": {
                        "name": file.filename,
                        "type": file_type,
                        "size": len(content)
                ...
                # Each patientId doc holds an array or history set; we append by creating a new subdoc
                db.collection("treatment_plans").document(patient_id).collection("history").add(plan_doc)
            except Exception as e:
                logger.warning(f"Failed to write treatment plan: {e}")

        # 6) Return structured envelope compatible with existing frontend
        return {
            "status": "success",
            "file_analysis": {
